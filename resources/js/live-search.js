...
export default class Search {
    ...
    async sendRequest() {
        try {
            const results = await axios.get(`/search/${this.inputField.value}`);
            this.renderResultsHTML(results.data);
        } catch (error) {
            console.error("Error fetching search results:", error);
            this.hideLoaderIcon();
            this.showResultsArea(); // Make sure to handle the loader and results area visibility in case of an error
        }
    }
